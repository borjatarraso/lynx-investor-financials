"""Command-line interface for lynx-finance."""
...
from lynx_finance import __author__, __author_email__, __license__, __version__, __year__, SUITE_LABEL
...
def build_parser():
    parser = argparse.ArgumentParser(
        prog="lynx-finance",
        description="Lynx Financials Analysis — Banks, Insurance, Asset Managers, Capital Markets & Fintech fundamental analysis tool.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=(
            "Examples:\n"
            "  lynx-finance -p JPM                             Mega-cap diversified bank\n"
            "  lynx-finance -p USB                             Super-regional bank\n"
            "  lynx-finance -p BLK                             Asset manager (BlackRock)\n"
            "  lynx-finance -p TRV                             P&C insurer (Travelers)\n"
            "  lynx-finance -p CME                             Capital-markets infra\n"
            "  lynx-finance -p MMC --refresh                   Force fresh data\n"
            '  lynx-finance -p "Goldman Sachs"                 Search by name\n'
            "  lynx-finance -p -i                              Interactive mode\n"
            "  lynx-finance -t JPM                             Testing mode\n"
        ),
    )
...
def run_cli():
    ...
    if "--b2m" in sys.argv:
        ...
        from lynx_finance.easter import rich_rocket, rich_fortune
        c = Console()
        rich_rocket(c)
        rich_fortune(c)
        return
    ...
    if "--about" in sys.argv:
        from rich.console import Console; from rich.panel import Panel; from lynx_finance import get_about_text
    ...
    if "--explain-section" in sys.argv:
        ...
        from lynx_finance.metrics.explanations import get_section_explanation, SECTION_EXPLANATIONS
    ...
    if "--explain-conclusion" in sys.argv:
        ...
        from lynx_finance.metrics.explanations import get_conclusion_explanation, CONCLUSION_METHODOLOGY
    ...
    if "--explain" in sys.argv:
        from rich.console import Console; from rich.panel import Panel; from rich.table import Table
        from lynx_finance.metrics.explanations import get_explanation, list_metrics
    ...
    from lynx_finance.core.storage import set_mode, is_testing
    ...
    if args.interactive:
        from lynx_finance.interactive import run_interactive; run_interactive(); return
    if args.tui:
        from lynx_finance.tui.app import run_tui; run_tui(); return
    if args.gui:
        from lynx_finance.gui.app import run_gui; run_gui(args); return
    if args.search:
        if not args.identifier: errc.print("[bold red]Error:[/] Provide a search query."); sys.exit(1)
        from lynx_finance.core.ticker import search_companies, display_search_results
    ...
    from lynx_finance.display import display_report_stage
    try:
        ...
        if args.export:
            from pathlib import Path; from lynx_finance.export import ExportFormat, export_report
        ...
        from lynx_finance.core.analyzer import SectorMismatchError
        if isinstance(e, SectorMismatchError):
            ...
            errc.print(Panel(
                ...
                f"[bold red]{e}[/]\n\n"
                f"[bold blink red]This tool is specialized ONLY for the Financials sector.[/]\n"
...
def _cmd_list_cache(con):
    from rich.table import Table; from lynx_finance.core.storage import list_cached_tickers, get_mode
...
def _cmd_drop_cache(con, target):
    from lynx_finance.core.storage import drop_cache_all, drop_cache_ticker, get_mode
    label = f"({get_mode()})"
    if target.upper() == "ALL":
